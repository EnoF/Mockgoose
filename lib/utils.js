...
/**
 * Traverse a object and find its deepest value, from this point, determine the
 * query and path to that value.
 *
 * @param  {Object} object
 * @return {Object}
 */
function getQueryAndPathForDeepValue(object) {
    // Nothing to do if value is not an object
    if(!_.isObject(object)) {
        return {query: object, pathToValue: null};
    }

        /**
         * Ref while traversing
         * @type {Object}
         */
    var currentRef = object,

        /**
         * Temporary key value while traversing
         */
        key,

        /**
         * List to hold all keys for path to value
         * @type {Array}
         */
        keyPath = [],
    ...
    /**
     * Walk the object, collecting all keys in the path list.
     */
    while(_.isObject(currentRef)) {
        key = _.first(_.keys(currentRef));
    ...
    // If the last path value was an operation, back up one step
    if(operations.isOperation(_.last(keyPath))) {
        keyPath.pop();
    }

    /**
     * Set the return object to the last key
     */
    returnObject[key] = currentRef;

    return {query: returnObject, pathToValue: keyPath};
}

/**
 * Extract a deeply nested value from an object identified by its property
 *
 * @param  {Object} object   Object to extract
 * @param  {Array|String} property Path to value
 * @return {Mixed}
 */
function extract(object, property) {
    // Clone the property value because function will perform destructive operations
    // on it and don't want to affect outside code.
    property = _.clone(property);

    var obj = object,
        parts = _.isString(property) ? property.split('.') : property,
        part = parts.shift();
...
function nestedValuesMatch(result, queryValue, item, query, q) {
    return _.every(_.keys(queryValue), function (key) {
        if (operations.isOperation(key)) {
            ...
            /**
             * Add a new condition to handle lookups that are nested deeper than
             * two.
             */
            if(_.has(query, q) && _.isObject(query[q][key])) {
                    /**
                     * From the
                     * @type {[type]}
                     */
                var toEvaluate = getQueryAndPathForDeepValue(query[q][key]),

                    /**
                     * The path to the value that will be accessed by the operation
                     * module
                     *
                     * @type {String}
                     */
                    newKey = toEvaluate.pathToValue,

                    /**
                     * First key of query should be an operation
                     * @type {String}
                     */
                    operation = _.first(_.keys(toEvaluate.query)),

                    /**
                     * The path to the deepest value
                     * @type {Array}
                     */
                    pathToValue = getQueryAndPathForDeepValue(query),

                    /**
                     * Item that the operation will received, primed and ready
                     * to be accessed on the newKey value.
                     */
                    itemToSendToOperation;

                if(!operation) {
                    return false;

                }
                // Need to get the path to the deepest value and back up one step
                pathToValue = pathToValue.pathToValue;
                pathToValue.pop();

                // Get the object that points to the deepest value with newKey
                itemToSendToOperation = extract(item, pathToValue);

                if(!operations.isOperation(operation)) {
                    return false;
                }

                return operations.getOperation(operation)(
                    resultObject(result, itemToSendToOperation),
                    toEvaluate.query,
                    {query: query, queryItem: newKey }
                );
